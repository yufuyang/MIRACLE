...
public interface CompanyProductMapper extends BaseMapper<CompanyProduct> {
    ...
    @Select({
            ...
            "    COALESCE(SUM(s.intention_count), 0) as intention_count",
            ...
            "    </if>",
            "    <if test='query.categoryType != null and query.categoryType != \"\"'>",
            "        AND p.category_type = #{query.categoryType}",
            "    </if>",
            "    <if test='query.companyId != null and query.companyId != \"\"'>",
            "        AND p.company_id = #{query.companyId}",
            "    </if>",
            "    AND p.status = 1",
            "</where>",
            ...
            "        ORDER BY SUM(s.view_count) <if test='query.asc'>ASC</if><if test='!query.asc'>DESC</if>",
            "    </when>",
            "    <when test='query.orderField == \"intentionCount\"'>",
            "        ORDER BY SUM(s.intention_count) <if test='query.asc'>ASC</if><if test='!query.asc'>DESC</if>",
            "    </when>",
            "    <otherwise>",
